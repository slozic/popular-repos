...
public class GithubRepoServiceTest {
    ...
    public void getPopularGithubRepos_shouldReturnSuccessfully() throws IOException {
        ...
        when(githubClient.getPopularGithubRepos(new QueryParameters(Optional.empty(), Optional.empty(), Optional.empty()).toGithubFilter())).thenReturn(githubRepositoriesListDto);

        // when
        PopularReposListDto mostPopularReposList = gitHubRepoService.getPopularGithubRepos(new QueryParameters(Optional.empty(), Optional.empty(), Optional.empty()).toGithubFilter());
    ...
    public void getPopularGithubRepos_shouldUseCache() throws IOException {
        ...
        // when
        PopularReposListDto mostPopularReposList = gitHubRepoService.getPopularGithubRepos(new QueryParameters(Optional.empty(), Optional.empty(), Optional.empty()).toGithubFilter());
        PopularReposListDto mostPopularReposList2 = gitHubRepoService.getPopularGithubRepos(new QueryParameters(Optional.empty(), Optional.empty(), Optional.empty()).toGithubFilter());
